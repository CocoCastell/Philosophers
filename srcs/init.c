...
/*   Updated: 2025/06/03 14:25:31 by cochatel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	return (set_int(get_time_in_ms() - philo->infos->time_start, \
				&philo->time_last_meal, &philo->meal_time));
}
...
t_philo	*init_philo(int nb_of_philo, t_mutex *forks, t_data *infos)
{
	...
	while (++i < nb_of_philo)
	{
		...
		if (safe_mutex_handler(MUTEX_INIT, &philos[i].meals_mutex) != 0 || \
				safe_mutex_handler(MUTEX_INIT, &philos[i].meal_time) != 0)
			return (NULL);
	}
	return (philos);
}
...
t_data	*init_data(char *argv[], int nb_of_arg)
{
	...
	if (safe_mutex_handler(MUTEX_INIT, &data->is_end_mutex) != 0 || \
			safe_mutex_handler(MUTEX_INIT, &data->print_mutex) != 0 || \
			safe_mutex_handler(MUTEX_INIT, &data->data_mutex) != 0)
		return (NULL);
