...
/*   Updated: 2025/06/03 14:04:46 by cochatel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_semaphores(t_data *data)
{
	...
	data->forks = sem_open("/forks", O_CREAT | O_EXCL, S_IRUSR | S_IWUSR, \
			philo_nb);
	data->philo_dead = sem_open("/philo_dead", O_CREAT | O_EXCL, S_IRUSR | \
			S_IWUSR, 1);
	data->write = sem_open("/write", O_CREAT | O_EXCL, S_IRUSR | S_IWUSR, 1);
	data->end_diner = sem_open("/end_diner", O_CREAT | O_EXCL, S_IRUSR | \
			S_IWUSR, 0);
	data->limit = sem_open("/limit", O_CREAT | O_EXCL, S_IRUSR | S_IWUSR, \
			philo_nb / 2);
	sem_unlink("/forks");
	...
	sem_unlink("/limit");
	if (data->limit == SEM_FAILED || data->end_diner == SEM_FAILED || \
			data->write == SEM_FAILED || data->philo_dead == SEM_FAILED \
...
t_data	*init_data(char *argv[], int nb_of_arg)
{
	...
	if (data->nb_of_meals == 0)
		return (free(data), printf("Error: Number of meals must be greater \
					than 0.\n"), NULL);
