...
/*   Updated: 2025/06/03 16:57:27 by cochatel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mutex_err_handler(int error, t_opcode opcode)
{
	...
	if (error == EINVAL && (opcode == MUTEX_LOCK || opcode == MUTEX_UNLOCK))
		...
		print_action(NULL, "Error: A deadlock would occur if the thread blocked waiting \
				for mutex.\n", false);
	else if (error == EPERM)
...
{
	if (c == PTHREAD_CREATE && thread_err_handler(pthread_create(t, \
					NULL, f, p), c) == 1)
		return (1);
	else if (c == PTHREAD_DETACH && thread_err_handler(\
				pthread_detach(*t), c) == 1)
		return (1);
	return (0);
}
