...
void	eating_routine(t_philo *philo)
{
	...
	if (set_long(get_time_in_ms(), &philo->time_last_meal, \
				&philo->meal_time) != 0)
...
void	philo_routine(t_philo *philo)
{
	...
	if (safe_th_handle(PTHREAD_CREATE, &death_thread, philo, is_dead) == 1)
		exit_error(philo, "Error creating death thread");
	if (safe_th_handle(PTHREAD_DETACH, &death_thread, NULL, NULL) == 1)
		exit_error(philo, "Error detaching death thread");
	while (true)
	{
		eating_routine(philo);
		if (philo->infos->nb_of_meals != -1 && philo->meals_eaten >= \
				philo->infos->nb_of_meals)
		{
			clear_all(philo->infos, 1);
			exit(0);
		}
...
int	diner_table(t_data *data)
{
	...
	i = -1;
	if (safe_th_handle(PTHREAD_CREATE, &kill_thread, data->philos, \
				kill_philos) == 1)
		return (1);
	if (safe_th_handle(PTHREAD_DETACH, &kill_thread, NULL, NULL) == 1)
		return (1);
